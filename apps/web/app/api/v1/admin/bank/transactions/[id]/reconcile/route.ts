...
import { bankPayoutSchema, bankTransactionIdSchema } from '@umunara/schemas'
...
export async function POST(
  ...
  return jsonResponse(async () => {
    ...
    const input = bankPayoutSchema.parse(await readJson(request))
    return createReconciliationService().linkPayout(actor, id, input.donationIds)
  })
}
